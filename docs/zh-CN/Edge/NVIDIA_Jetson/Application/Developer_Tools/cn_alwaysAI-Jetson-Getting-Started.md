...
[alwaysAI](https://alwaysai.co) 是一个重要的计算机视觉开发平台，用于在 NVIDIA® Jetson 等边缘设备上创建和部署机器学习应用程序。开发和部署这些应用程序通常过于复杂且耗时，需要深入了解大量计算机视觉技术和方法，这些知识更适合专家而非普通开发者。alwaysAI 消除了这些障碍，使得创建计算机视觉应用程序变得简单、快速且高效。
...
- 运行 JetPack 4.6 的上述任意 Jetson 设备，并安装所有 SDK 组件（安装参考请查看 [此 Wiki](https://wiki.seeedstudio.com/cn/reComputer_J1020_A206_Flash_JetPack/)）
- 一台运行 Windows、Linux 或 Mac 的主机电脑
...
:::note
在本指南中，我们将使用 Windows 开发电脑。然而，如果您想将 Mac 或 Linux 设置为开发电脑，请 [点击这里](https://alwaysai.co/docs/get_started/development_computer_setup.html) 了解更多。
:::

- **步骤 1.** 下载并安装 alwaysAI 桌面应用程序和命令行界面，点击 [这里](https://alwaysai.co/installer/windows)
...
如果您看到打印的版本号，则说明您已成功安装 alwaysAI CLI。本指南撰写时的版本号为 **1.4.3**。
...
如果您看到打印的版本号，则说明 OpenSSH 已安装。例如 **OpenSSH_for_Windows_8.1p1, LibreSSL 3.0.2**。自 2018 年底起，Windows 10 已原生支持 OpenSSH。有关在 Windows 上安装 OpenSSH 的更多信息，请查看 Microsoft 文档网站上的 [这篇文章](https://docs.microsoft.com/en-us/windows-server/administration/openssh/openssh_install_firstuse)。

### 设置 Jetson 设备的环境

接下来，您需要在边缘设备（本例中为 Jetson 设备）上设置环境。

**步骤 1.** 在 Jetson 设备上运行机器学习项目时，使用的是 edgeIQ 运行时，它作为 [Docker 镜像](https://hub.docker.com/r/alwaysai/edgeiq) 提供。您无需安装 Docker，因为它已随 JetPack 预装。然而，您需要将 "docker" 组添加到您的用户中，以便无需 root 权限（sudo）即可访问 Docker。访问 Jetson 设备并在终端中执行以下命令：

```sh
sudo usermod -aG docker $USER
```

**步骤 2.** 要应用新的组成员身份，请注销 Jetson 设备并重新登录，或者输入以下命令：
...
**步骤 2.** 选择 **Object Detection（目标检测）** 作为起始模板，输入项目名称并点击 **Create Project（创建项目）**

<div align="center"><img width="{550}" src="https://files.seeedstudio.com/wiki/alwaysAI/pics/5.png"/></div>

**步骤 3.** 点击 **Click Here To View Your Project（点击此处查看您的项目）** 进入新创建的项目

现在您将看到有关新创建项目的信息，例如使用的模型和其他有用的细节。

<div align="center"><img width= "{1000}" src="https://files.seeedstudio.com/wiki/alwaysAI/pics/6.png"/></div>

对于默认的 **Object Detection Project（目标检测项目）**，使用的模型是 **mobilenet_ssd**，它是使用 **Pascal VOC 数据集**训练的。这意味着，该应用程序可以识别 **20 种对象类别**，例如人、鸟、猫、牛、狗、马、羊、飞机、自行车、船、公交车、汽车、摩托车、火车、瓶子、椅子、餐桌、盆栽植物、沙发、电视/显示器。

然而，该模型并未针对 Jetson 硬件进行优化，因此运行时帧率会非常低。因此，我们将选择一个针对 **Jetson Xavier NX** 优化并支持 **TensorRT** 的模型。

**步骤 4.** 点击模型旁边的三个点，然后点击 **Delete（删除）**

<div align="center"><img width= "{1000}" src="https://files.seeedstudio.com/wiki/alwaysAI/pics/32.png"/></div>

**步骤 5.** 点击 **Add New Model（添加新模型）**，然后点击 **Go to Model Catalog（前往模型目录）** 进入 alwaysAI 模型目录
...
此模型使用 **COCO 数据集**训练，可以识别 **80 种对象类别**，例如人、自行车、汽车、摩托车、飞机、公交车、火车、卡车、船、交通信号灯、消防栓、停车标志、停车计时器、长椅、鸟、猫、狗、马、羊、牛、大象、熊、斑马、长颈鹿、背包、雨伞、手提包、领带、手提箱、飞盘、滑雪板、单板滑雪、运动球、风筝、棒球棒、棒球手套、滑板、冲浪板、网球拍、瓶子、酒杯、杯子、叉子、刀、勺子、碗、香蕉、苹果、三明治、橙子、西兰花、胡萝卜、热狗、披萨、甜甜圈、蛋糕、椅子、沙发、盆栽植物、床、餐桌、厕所、电视、笔记本电脑、鼠标、遥控器、键盘、手机、微波炉、烤箱、烤面包机、水槽、冰箱、书、时钟、花瓶、剪刀、泰迪熊、吹风机、牙刷。

**步骤 7.** 选择之前创建的项目（此处为 My First Project），然后点击 **Add To Project（添加到项目）**
...
现在我们将通过开发计算机使用 SSH 将之前创建的项目部署到 Jetson 设备上。

**步骤 1.** 在开发计算机上创建一个新文件夹，在新文件夹中打开命令行界面并输入以下内容：
...
:::note
首次运行上述命令时，系统会提示您输入 alwaysAI 账户的用户名和密码。
:::
...
**步骤 5.** 点击 **Add a new device（添加新设备）** 将您的 Jetson 设备添加为远程设备
...
**步骤 8.** 输入 Jetson 设备的 **用户名** 和 **主机名/IP 地址**，如下所示：
...
**步骤 11.** 在 PC 上为 alwaysAI 创建的目录中，打开 **app.py** 并按如下所示更改模型名称和推理引擎类型：
...
:::note
如果您在此步骤中遇到错误，可以尝试使用 `aai app install --clean` 方法重新部署。请仔细检查 Jetpack 版本是否为**要求的版本 4.6**。您可以通过输入命令 `sudo apt-cache show nvidia-jetpack` 来检查版本号。如果版本号正确，请确保您完成了 Jetson 系统安装以及**Jetson SDK 组件**的安装。这将帮助您解决大多数问题。
:::
...
**步骤 3.** 打开一个网页浏览器并输入 **http://localhost:5000** 以打开视频流
...
如您所见，系统正在实时检测人员，并在界面上显示每个标签的置信度百分比。此外，在 **Jetson Xavier** 上的推理时间为 **0.009 秒，大约为 111 fps**。
...
**步骤 1.** 将视频文件放置在 Jetson 设备的 alwaysAI 项目目录中。根据我们的示例，路径为 **/home/`<username>`/alwaysai/test**
...
此时，您将在命令行上看到与之前相同的输出，并且一旦您使用之前提到的地址打开网页浏览器，您将看到在预加载视频上执行的目标检测，输出与之前类似。

### 筛选要检测的目标

您还可以筛选模型要检测的特定目标。例如，上述 COCO 数据集可以检测 80 种目标类型。然而，我们可以筛选仅检测人员。
...
alwaysAI 在其 GitHub 仓库中提供了多种现成可用的应用程序。你可以在[这里](https://github.com/alwaysai/Reference-Applications)查看不同的应用程序。

### 车牌检测器

为了演示，我们将说明如何将其中一个应用程序 [License Plate Detector（车牌检测器）](https://github.com/alwaysai/license-plate-detector) 部署到 Jetson 设备上。你也可以对其他应用程序重复相同的步骤。

**步骤 1.** 在开发计算机上，为我们的项目创建一个新文件夹并进入该文件夹

**步骤 2.** 下载[这个仓库](https://github.com/alwaysai/license-plate-detector)为 **.zip** 文件，或者如果你的电脑上安装了 **Git**，可以克隆它

默认情况下，此示例中使用的模型未针对 Jetson Nano 进行优化。因此，我们首先需要加载一个针对 Jetson Nano 优化的模型。
...
如你所见，我们正在使用 **vehicle_license_mobilenet_ssd 模型**，该模型能够识别车辆和车牌。

**步骤 11.** 在你的电脑上打开一个网页浏览器，并输入以下地址以打开视频流
...
在此演示中，推理是在[这里](https://github.com/alwaysai/license-plate-detector/tree/main/video)的两个视频上进行的。如你所见，视频流中的车辆和车牌正在被检测到，并且每个标签的置信度百分比显示在界面上。

### 使用摄像头进行推理

如果你想对来自摄像头的实时视频流执行与上述相同的推理，请按照以下步骤操作
...
此时你将在命令行上看到与之前相同的输出，并且一旦你使用之前提到的相同地址打开网页浏览器，你将看到对象检测正在对来自摄像头的实时视频流进行处理，并生成与之前类似的输出。
...
**步骤 1.** 准备您的训练数据集。确保您的数据集采用 **PascalVOC 格式**，然后将所有图像和标签分别放入两个目录中，并按如下方式压缩文件：

<div align="center"><img width="{350}" src="https://files.seeedstudio.com/wiki/alwaysAI/pics/27.jpg"/></div>

**步骤 2.** 访问 [alwaysAI 控制台](https://console.alwaysai.co/dashboard)，点击左侧导航栏中的 **Datasets**，然后点击 **Upload a Dataset**。
...
现在，训练好的模型已存储在您的账户中。您可以在对象检测项目中使用此模型，具体步骤如下：

**步骤 5.** 导航到 `Models > My Models`，然后点击刚刚训练的模型。
...
**步骤 6.** 如下所示，您可以在对象检测应用的根目录中使用以下命令：

```sh
aai app models add lakshanthad/roadsign-mobilenet
```

**步骤 7.** 然后，在 **app.py** 中更改模型名称并重新运行应用：
...
感谢您选择我们的产品！我们为您提供多种支持渠道，以确保您在使用我们的产品时获得顺畅的体验。我们提供多种沟通渠道，以满足不同的偏好和需求。
