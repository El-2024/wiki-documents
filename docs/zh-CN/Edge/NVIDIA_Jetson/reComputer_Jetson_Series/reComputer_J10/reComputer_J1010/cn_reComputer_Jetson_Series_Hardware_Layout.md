...
在这里，您将了解 Jetson 系列 reComputer 的基本硬件布局。同时，您还将学习如何从载板上拆卸和安装核心板、如何安装摄像头、M.2 Wi-Fi 模块或硬盘等。

## reComputer 系列放置方式

当面向 reComputer 接口的背板时，机箱右侧有 4 个防滑垫，便于竖立，如下图所示。
...
reComputer 机箱底部为悬空结构，可通过绑带固定在一些不便的结构上。机箱底板有 4 个固定孔，便于固定在立面或斜面上。
...
当面向 reComputer 接口的背板时，机箱右侧有 4 个防滑垫，可以看到一个金属按钮，如下图所示：
...
打开机箱后，您可以看到插在载板上的模块，如下图所示。按照以下步骤可以将模块从载板上拆下。
...
- **步骤 1**. 找到载板上的 Jetson SODIMM 接口触点以及模块上的对应接口。
...
- **步骤 2**. 将模块以大约 20 度的角度斜插入载板的插槽中。向下按压后，模块将固定在载板上。
...
!!! 注意
    正确安装后，您可能会发现接口处露出少量触点。如果暴露过多触点，模块将不稳定。
...
## 从 reComputer 机箱中移除载板

当我们需要安装 M.2 模块或 CSI 摄像头等设备时，需要将载板从 reComputer 机箱中移除以便操作。载板通过 4 个螺丝固定在机箱底座上，如下图所示：
...
reComputer 的载板均配备两个 CSI 接口。这些接口通常用于连接摄像头以完成一些识别项目。以下以 J1010 载板为例，指导您如何在 reComputer 上安装和使用 [Raspberry Pi Camera Module V2](https://www.seeedstudio.com/Raspberry-Pi-Camera-Module-V2.html)。

- **步骤 1**. 将摄像头模块安装到载板上

!!! 注意
    安装前，请关闭 reComputer，拔掉电源，并打开机箱顶部盖板。
    为了方便演示，我们将载板从机箱和模块中移除。实际操作只需打开顶部盖板。

选择您要使用的 CSI 接口，然后轻轻翻起两侧的黑色固定槽。
...
注意排线的方向。您可以看到排线的金属触点面朝向载板，而蓝色面朝外。
...
插入摄像头排线后，确保模块、载板和外设均已安装到位，然后开机。

- **步骤 3**. 检查摄像头是否被识别。

在命令行窗口中输入以下命令，查看当前是否有摄像头设备可用。

```shell
ls /dev/video0 
```

如果输出如下所示，则说明摄像头已成功检测到。

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reComputer-Jetson-Nano/50.jpg" /></div>

如果未看到设备文件，请检查排线方向是否正确，并确保您的 Raspberry Pi 摄像头为 V2 版本，因为 V1 版本无法被识别。
...
# 简单测试
# 按 Ctrl^C 退出
# sensor_id 选择摄像头：Jetson Nano B01 上为 0 或 1
$ gst-launch-1.0 nvarguscamerasrc sensor_id=0 ! nvoverlaysink
```

您可以在命令行窗口中运行以下命令测试摄像头的使用。
...
- 2 个 IPEX 转 SMA 母外置天线适配器和 SMA 公天线（用于 WIFI 模块）
- 十字螺丝刀和螺丝
...
!!!注意
    安装 M.2 Key E 的无线模块需要额外的天线。由于模块位于机箱内，甚至可能被压在模块和载板之间。如果没有天线，信号强度将受到极大影响。

- **步骤 1**. 从 reComputer 机箱中拆下载板

在安装无线模块之前，请按照下图所示从 reComputer 机箱中拆下载板：
...
选择两个外侧的孔，从机箱外部将硅胶塞挤压到机箱内部，然后从内部拉出硅胶塞以露出天线孔。
...
如图所示，将 SAM 接头的螺母安装到 WiFi 孔上，注意将电缆端放置在机箱内。
...
!!!注意
    M.2 Key E 接口位于载板的底部或顶部。对于不同的载板，请参考载板硬件布局。在安装之前，您可能需要先将核心板从载板上移除。

如图所示，在载板上找到 M.2 Key E 接口，并将无线模块插入接口插槽。
...
安装完成后应如图所示。此时，请小心不要弄断无线模块与 SAM 天线底座之间的线缆。
...
将两个 SAM 公天线安装到 SAM 母插座上并拧紧。至此，所有硬件安装完成。
...
*点击[这里](https://files.seeedstudio.com/wiki/recomputer-Jetson-20-1-H1/A206-carrier-board.pdf)查看 Jetson A206 载板（搭载 Jetson Nano）的引脚与操作接口。*
...
|   千兆以太网       |                                         1x RJ45 千兆以太网接口 (10/100/1000)                                         |              1x RJ45 千兆以太网接口 (10/100/1000)               | 1x RJ45 千兆以太网接口 (10/100/1000)                                                                                | 1x RJ45 千兆以太网接口 (10/100/1000)                                                                                |
|          USB       | 1 x  USB 3.0 Type A 接口；  2 x  USB 2.0 Type A 接口； 1 x  USB Type C 用于设备模式； 1 x  USB Type C 用于 5V 电源输入 |    4 x  USB 3.0 Type A 接口； 1 x  Micro-USB 接口用于设备模式；    |                               4 x  USB 3.0 Type A 接口； 1 x  Micro-USB 接口用于设备模式；                               |                               4 x  USB 3.0 Type A 接口； 1 x  Micro-USB 接口用于设备模式；                               |
|  CSI 摄像头接口 |                                          2x CSI 摄像头 (15针, 1mm间距, MIPI CSI-2 )                                         |               2x CSI 摄像头 (15针, 1mm间距, MIPI CSI-2 )              | 2x CSI 摄像头 (15针, 1mm间距, MIPI CSI-2 )                                                                                  | 2x CSI 摄像头 (15针, 1mm间距, MIPI CSI-2 )                                                                                  |
...
|       M.2 KEY E     |                                                           1x M.2 Key E                                                          |                          1x M.2 Key E（禁用）                         |                                                           1x M.2 Key E                                                          |                                                           1x M.2 Key E                                                          |
|       M.2 KEY M     |                                                                -                                                               |                                1x M.2 Key M                               | 1x M.2 Key M                                                                                                                    | 1x M.2 Key M                                                                                                                    |
|          RTC        |                                                          1x RTC 插槽                                                          |                               1x RTC 插槽                               |                                                          1x RTC 插槽                                                          |                                                          1x RTC 插槽                                                          |
| 多功能接口          |                                                        1x  40针排针                                                        |                             1x  40针排针                             | 1x  40针排针                                                                                                               | 1x  40针排针                                                                                                               |
|  电源               |                                                       USB-Type C 5V⎓3A；                                                       |                              DC 插孔 12V/2A                              | DC 插孔 19V/4.74A (最大 90W)                                                                                                      | DC 插孔 19V/4.74A (最大 90W)                                                                                                      |
...
|  USB Micro Type B        |  -                                               |  1x USB Micro B，RA 母口                      |
...
|  多功能接口              |  2.0 间距 40 针                                |  2.0 间距 40 针                                |
|  按键接口                |  1x 按键接口 (1x12，2.54mm 间距，RA)           |  1x 按键接口 (1x12，2.54mm 间距，RA)           |
|  风扇接口                |  1x Picoblade 接口                              |  1x Picoblade 接口                              |
|  CAN 总线                |  禁用                                           |  1x CAN 总线接口 (1x4，2.54mm 间距，RA)        |
|  RTC                     |  1x RTC 备用纽扣电池插座 (CR1220)              |  1x RTC 备用纽扣电池插座 (CR1225)              |
|  电源接口                |  1x Type C 接口                                |  1x DC 输入电源 TE 接口                        |
