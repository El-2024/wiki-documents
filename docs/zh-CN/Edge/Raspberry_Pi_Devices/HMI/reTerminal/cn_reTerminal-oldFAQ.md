...
## 问题1：如何更新 reTerminal LCD 的 STM32 固件
...
- **方法 2：** 使用跳线将 STM32 芯片引脚物理连接到 40 针 reTerminal GPIO，然后使用 OpenOCD 刷写固件

如果您的 reTerminal 上的 STM32 固件是**新版本（v1.7 或更高版本）**，则可以使用**方法 1**；如果您的板载 STM32 固件是**旧版本（低于 v1.7）**，则需要使用**方法 2**。

### 确定使用哪种刷写方法

现在让我们按照以下步骤确定板载固件的版本，以便选择合适的刷写方法。
...
**注意：** 这将卸载所有 reTerminal 驱动程序。因此，下次启动 reTerminal 时，所有驱动程序都不会加载。
...
- **步骤 4.** 通过 **i2c-tools** 使 STM32 进入**引导模式**：
...
如果您可以看到 I2C 地址为 **0x56**，如下面的表格所示，则说明您的板载 STM32 固件是**新版本（v1.7 或更高版本）**。
...
然而，如果您看到 I2C 地址为 **0x45**，如下面的表格所示，则说明您的板载 STM32 固件是**旧版本（低于 v1.7）**。
...
如果您的板载 STM32 固件是**新版本（v1.7 或更高版本）**，请按照此方法操作。
...
<!-- - **步骤 5.** 访问 [此链接](https://github.com/Seeed-Studio/seeed-linux-dtoverlays/releases)，下载 **stm32flash** 文件和 **STM32G030F6_R2.bin** 文件（最新发布版本）。
...
- **步骤 8.** 通过 **i2c-tools** 使 STM32 进入**引导模式**：
...
**注意：** **pi** 是用户名，**192.168.x.xx** 是 reTerminal 的 IP 地址。您也可以用 reTerminal 的主机名替换。
...
如果板载的 STM32 固件是**旧版本（低于 v1.7）**，请按照以下方法操作。

- **步骤 1.** 打开 reTerminal 的终端窗口并输入以下命令更新软件包列表
...
- **步骤 4.** 访问[此链接](https://github.com/Seeed-Studio/seeed-linux-dtoverlays/releases)，从**最新发布版本**中下载 **STM32G030F6_R2.bin** 文件。
...
- **步骤 7.** 回到 reTerminal 的终端窗口，在 **openocd** 目录中输入以下命令
...
- **步骤 11.** 按照下图连接 STM32 的引脚到 40 针 GPIO
...
**注意：** STM32 的引脚位于 reTerminal PCBA 的背面。
...
**注意：** 通常烧录大约需要 3 秒。因此你需要**保持**上述连接约**3 秒**，直到烧录过程完成。
...
- **步骤 1.** 打开 reTerminal 的终端窗口并输入以下命令打开配置文件
...
- **步骤 4.** 在 reTerminal 的终端窗口中输入以下命令检查 STM32 固件版本
...
## Q2: 如何刷写原装随 reTerminal 附带的 Raspberry Pi OS？

如果您已经刷写了其他操作系统，并希望切换回随 reTerminal 附带的默认 Raspberry Pi OS，可以按照以下步骤操作：
...
**注意：** reTerminal 原装附带的是 32 位操作系统，但您也可以下载 64 位版本。
...
如果您希望在 reTerminal 上使用没有 eMMC 的 Compute Module 4，则需要插入 micro-SD 卡并刷写您选择的操作系统。根据您的操作系统，按照以下步骤操作：

- **步骤 1.** 使用 **micro-SD 卡读卡器**连接到电脑，或者使用笔记本电脑的**内置读卡器**，将 micro-SD 卡插入电脑。

- **步骤 2.** 通过 [此链接](https://www.raspberrypi.org/software/) 下载 **Raspberry Pi Imager** 软件。
...
**注意：** 您可以选择操作系统，例如 **64 位 Ubuntu**，通过导航到 **Other general purpose OS**。
...
- **步骤 8.** 从电脑中弹出 micro-SD 卡，并将其插入 reTerminal。
...
## 问题5：如何使用 USB 转串口转换器登录到 Raspberry Pi OS/Ubuntu OS 或其他操作系统
...
- **步骤 6.** 在 **Connection Type** 中选择 **Serial**。
...
- **步骤 7.** 按照上述硬件连接后，从墙壁电源插座打开电源以启动 reTerminal。

现在您已成功登录到 Raspberry Pi OS。

## 问题6：我无法唤醒 reTerminal 的 LCD 屏幕

通过 SSH 或 VNC 连接后，打开一个终端窗口并输入以下命令：
...
- **步骤 2.** 拆下散热片的 2 个螺丝，以及其余 4 个螺丝以拆开整个外壳
...
- **步骤 4.** 在主机电脑中打开一个 **终端** 窗口，连接到 reTerminal 后输入以下命令以更新 **软件包列表**
...
- **步骤 15.** 通过 USB Type-C 电缆将 reTerminal 连接到电脑

现在需要几秒钟时间将必要的文件传输到 reTerminal。

- **步骤 16.** 关闭 reTerminal，将启动模式开关拨回原始位置，并重新组装 reTerminal 外壳

- **步骤 17.** 连接一个包含合适操作系统的可启动 USB 闪存驱动器，将其连接到 reTerminal 的一个 USB 端口，然后打开 reTerminal

现在您将看到 reTerminal 从连接的 USB 驱动器启动。

## Q8: 为什么 CM4 上的芯片周围有黑色胶水？

我们特别在 CM4 的芯片周围涂抹了 **环氧树脂底填胶粘剂**（看起来像黑色胶水），以确保 IC 得到良好的保护。这也使整个 CM4 更加可靠。

## Q9: 为什么安装了 reTerminal 驱动后蜂鸣器、LED 和按钮无法工作？

reTerminal 配备了两种版本的 I/O 扩展芯片。旧版本使用 MCP23008，新版本使用 PCA9554。如果您是[手动安装了 reTerminal 驱动](https://wiki.seeedstudio.com/cn/reTerminal/#install-reterminal-drivers-after-flashing-new-raspberry-pi-os-ubuntu-os-or-other-os)，请检查以下内容。

首先，检查您的 reTerminal 上安装的是哪种芯片。
...
- 在文件末尾添加以下内容：`dtoverlay=reTerminal,addr=0x20,mcp23008`
...
**注意：** 请参考[此处](https://wiki.seeedstudio.com/cn/reTerminal/#flash-raspberry-pi-os-64-bit-ubuntu-os-or-other-os-to-emmc)的刷写说明。在打开 **Raspberry Pi Imager** 后，导航到 `CHOOSE OS > Use custom`，然后选择上述镜像进行刷写。

- **步骤 2.** 通过 SSH 登录到 reTerminal，并依次运行以下命令。请确保使用 **ubuntu** 作为用户名，**ubuntu** 作为密码。
...
- **步骤 3.** 将以下内容复制到上述文件中，以显式设置 LCD 显示屏 (DSI-1)，然后按 **ESC** 键后输入 **:wq** 保存文件
...
在将屏幕配置为正确方向后，触摸位置可能仍然不准确，当您触摸显示屏上的特定区域时，光标可能会朝意外方向移动。为了解决此问题，请按照以下步骤操作。
...
- **步骤 2** : 您可以访问 "40-libinput.conf" 文件，可以通过以下命令编辑该文件。
...
- **步骤 5** : 按 Ctrl+O 保存并按回车键，然后按 Ctrl+X 退出，之后重启

```sh
sudo reboot 
```

重启后，您可能会注意到触摸位置现在准确了。这意味着当您触摸显示屏上的特定区域时，光标会按照预期方向移动。

## Q13: 安装 reTerminal 驱动程序后 LED 和蜂鸣器无法工作

此问题不会持续很久。我们将在 reTerminal 驱动程序中完全解决此问题。
...
要检查加密芯片的类型，可以在终端中输入命令 ```i2cdetect -y 3```。如果在输出表中看到 ```0x35```，则说明 reTerminal 配备的是 ATECC608A-TNGTLSS-G 芯片，否则配备的是 ATECC608A-SSHDA-B。
