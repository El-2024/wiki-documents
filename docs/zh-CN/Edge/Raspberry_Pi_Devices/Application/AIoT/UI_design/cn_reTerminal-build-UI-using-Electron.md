---
description:  Electron 用于 reTerminal
title:  Electron 用于 reTerminal
keywords:
  - Edge
  - reTerminal 应用
...
本篇 Wiki 讲解了如何使用 Electron 构建您自己的用户界面。Electron 是一个开源框架，用于使用 HTML、CSS 和 JavaScript 等 Web 技术创建原生桌面应用程序（支持 Windows、Mac 和 Linux）。这意味着如果您可以构建一个网站，那么您也可以构建一个桌面应用程序！

通过以下指南，您将能够创建一个应用程序，只需点击 LCD 上的按钮即可控制 reTerminal 的 GPIO 引脚。那么让我们开始吧！
...
- **步骤 1.** 使用 **板载 LCD、外接显示器或 SSH** 访问 reTerminal，具体操作请参考[这里](https://wiki.seeedstudio.com/cn/reTerminal/#log-in-to-raspberry-pi-os-ubuntu-os-or-other-os-using-ssh-over-wi-fi-ethernet)
...
Node.js 现在已安装在 reTerminal 上。要检查安装是否成功，请运行以下命令检查 **Node.js** 和 **npm** 的版本：
...
现在，我们将为开发设置 Microsoft Visual Studio Code。

- **步骤 1.** 下载并安装 [Microsoft Visual Studio Code](https://code.visualstudio.com/)

**注意：** 下载适合您操作系统的安装程序

- **步骤 2.** 点击左侧导航菜单中的 **扩展** 选项卡，在搜索框中输入 **remote development**
...
现在我们将构建一个 **智能灯 Electron 应用程序**，通过按下 LCD 上的按钮来控制 reTerminal 的 GPIO。随后，您可以将继电器连接到 GPIO 并控制家用电器！

### 硬件连接

我们将把一个 **LED** 连接到 reTerminal 的 **GPIO 24** 进行测试。之后，您可以添加继电器并使用 GPIO 控制家用电器！
...
- **步骤 1.** 在主机电脑上打开 VSCode，并按照之前的说明通过 SSH 登录到 reTerminal
...
- **步骤 4.** 导航到 `Terminal > New Terminal`，然后进入新创建的文件夹
...
- **步骤 5.** 在终端窗口中输入以下命令以创建一个 **package.json** 文件，并为我们的 Node.js 应用程序设置所需的配置

```sh
npm init
```

**注意：** 按 **ENTER** 使用默认答案，但将 **entry point: (index.js)** 设置为 **main.js**，并将 **test command:** 设置为 **electron .**（在 **electron** 后使用一个空格和一个点）
...
- **步骤 6.** 在 VSCode 的终端中输入以下命令以安装 **Electron**

```sh
npm install --save-dev electron
```

如果成功安装 Electron，您将看到以下输出
...
您还会看到生成的 **node_modules** 文件夹，其中包含运行 Electron 所需的包
...
- **步骤 1.** 按照之前的说明在 VSCode 中打开一个终端窗口，并导航到主应用程序目录

- **步骤 2.** 输入以下命令以安装 **onoff** npm 模块
...
electron-rebuild npm 模块会根据您的 Electron 项目使用的 Node.js 版本重建原生 Node.js 模块。这使您可以在 Electron 应用程序中使用原生 Node.js 模块，而无需系统版本的 Node.js 完全匹配（这通常不可能，有时甚至无法实现）。
...
**注意：** 每次安装新的 npm 包时，请重新运行 electron-rebuild

### 创建 HTML（基本用户界面）

我们将使用 HTML 文件创建基本用户界面，不包含任何样式。此 HTML 文件负责在屏幕上显示 UI 元素。

在主应用程序目录中，创建一个名为 **index.html** 的新文件，并复制以下代码
...
**注意：** **ipcRenderer 模块**是一个事件触发器。它提供了一些方法，可以让您从渲染进程（网页 - HTML/CSS）向主进程（main.js）发送消息。您还可以接收来自主进程的回复。
...
/* 使用 ipcMain 模块接收来自 ipcRenderer 模块的消息并
打开 GPIO */
...
由于我们已经启用了 **electron-rebuild**，保存前面的文件后，您将立即看到输出。如果您关闭了应用程序，可以通过运行 **npm test** 再次打开它，并查看以下输出：
...
**注意：** X11 需要将 reTerminal 显示转发到 PC 上并弹出窗口
...
现在使用 **npm test** 运行文件一次，应用程序将在您保存文件时自动更新。您无需每次更改项目内容时都运行 **npm test**。
