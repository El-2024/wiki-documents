---
description: Azure IoT Edge Runtime 在 reTerminal DM 上运行
title: Azure IoT Edge Runtime 在 reTerminal DM 上运行
...
<div style={{textAlign:'center'}}><iframe width={270} height={358} frameBorder={0} src="https://devicecatalog.azure.com/embed/c40637ad-a9bf-494e-8975-f4d37e43cf6f" title="reTerminal DM - Azure 认证设备" /></div>


## 简介

reTerminal DM 是一款集面板电脑、HMI、PLC、IIoT 网关于一体的设备，由 Raspberry Pi CM4 提供支持，配备 10.1 英寸 IP65 前面板和丰富的工业接口，原生集成了 Node-RED，并支持基于 Raspberry Pi 的软件生态系统。

本文档描述了如何将运行 Debian 11 (ARM32v7) 的 reTerminal DM 设备连接到预装了 Azure IoT Edge Runtime 和设备管理功能的环境中。此多步骤过程包括：
...
:::tip
如果您的操作系统版本低于 Debian 11 (Bullseye) 或 LOG_BIT 为 64，请安装最新的 Raspberry Pi OS 32 位版本。请参考 [刷新 Raspbian OS 的步骤](https://wiki.seeedstudio.com/cn/reterminal-dm-flash-OS/#steps-for-flashing-raspbian-os)。
:::

:::tip
如果 `uname -m` 显示为 aarch64，说明您的操作系统正在运行 64 位内核。请切换到 32 位内核。参考 [32 位 OS 驱动](https://wiki.seeedstudio.com/cn/reterminal-dm-flash-OS/#32-bit-os-driver)。
:::

## 第 3 步：在设备上手动测试 Azure IoT Edge
...
在 IoT Edge 设备上打开命令提示符，确认从云端部署的模块正在 IoT Edge 设备上运行。
