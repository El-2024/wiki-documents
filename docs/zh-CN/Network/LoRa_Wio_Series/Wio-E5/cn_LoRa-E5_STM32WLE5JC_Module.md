---
description: Wio-E5 STM32WLE5JC 模块快速入门。
...
<div style={{textAlign: 'center'}}>
    <a href="https://www.seeedstudio.com/LoRa-E5-Wireless-Module-p-4745.html"><strong><span><font color={'FFFFFF'} size={"4"}> 立即购买 🖱️</font></span></strong></a>
</div>
...
:::note
我们最近发布了基于 Wio-E5 模块的 Wio-E5 系列产品。点击[这里](https://www.seeedstudio.com/lora-c-755.html?product_list_stock=3)了解 Wio-E5 家族的新成员，包括 [Grove 模块](https://wiki.seeedstudio.com/cn/Grove_LoRa_E5_New_Version/)、[迷你开发板](https://wiki.seeedstudio.com/cn/LoRa_E5_mini/) 和 [开发套件](https://wiki.seeedstudio.com/cn/LoRa_E5_Dev_Board/)。  
想了解如何使用 STM32WL 系列的 STM32Cube MCU 软件包 (SDK) 创建 LoRaWAN 终端节点，加入并向 LoRaWAN 网络发送数据，请参阅 [迷你开发板](https://wiki.seeedstudio.com/cn/LoRa_E5_mini/) 和 [开发套件](https://wiki.seeedstudio.com/cn/LoRa_E5_Dev_Board/) 的 Wiki 页面。
:::

Wio-E5 是由 Seeed Technology Co., Ltd. 设计的一款低成本、超低功耗、极其紧凑且高性能的 LoRaWAN® 模块。  
**它包含 ST 系统级封装芯片 STM32WLE5JC，这是全球首款集成 LoRa® RF 和 MCU 芯片的 SoC。**  
该模块还嵌入了 ARM Cortex M4 超低功耗 MCU 和 LoRa® SX126X，因此支持 (G)FSK 模式和 LoRa®。在 LoRa® 模式下，可使用 62.5kHz、125kHz、250kHz 和 500kHz 带宽，适用于各种物联网节点设计，支持 EU868 和 US915 频段。

Wio-E5 模块采用工业标准设计，非常适合用于工业物联网产品设计，工作温度范围宽广，为 -40℃ ~ 85℃。

如果您对 LoRa® 和 LoRaWAN® 不太熟悉，可以查看这篇文章 [LoRapedia](https://www.seeedstudio.com/blog/2020/08/03/lorapedia-an-introduction-of-lora-and-lorawan-technology/) 了解详细信息。
...
* **高性能**：TXOP=22dBm@868/915MHz；SF12（125KHz 带宽）灵敏度为 -136.5dBm

* **远距离使用**：158dB 链路预算

* **无线连接**：内嵌 LoRaWAN® 协议，支持 AT 命令，兼容全球 LoRaWAN® 频率计划

* **全球兼容性**：宽频率范围；支持 EU868 / US915 / AU915 / AS923 / KR920 / IN865

* **极大的灵活性**：对于希望在模块 MCU 上开发软件的用户，MCU 的其他 GPIO 可以轻松操作，包括 UART、I2C、ADC 等。这些丰富的 GPIO 接口对于需要扩展外设的用户非常有用。

* **通过 FCC、CE、IC 和 Telec 认证**

## 应用场景

* 适用于 LoRaWAN® 传感器节点及任何无线通信应用。
...
Wio-E5 系列内置 AT 命令固件，支持 LoRaWAN® Class A/B/C 协议以及广泛的频率计划：EU868/US915/AU915/AS923/KR920/IN865。借助此 AT 命令固件，开发者可以轻松快速地构建原型或应用。

AT 命令固件包含用于 DFU 的引导加载程序和 AT 应用程序。"PB13/SPI_SCK/BOOT" 引脚用于控制 Wio-E5 是停留在引导加载程序还是跳转到 AT 应用程序。当 PB13 为高电平时，模块在复位后会跳转到 AT 应用程序，默认波特率为 9600。当 PB13 为低电平（按下 Wio-E5 mini/Wio-E5 开发套件上的 "Boot" 按钮），模块会停留在引导加载程序中，并以波特率 115200 每秒发送一次 "C" 字符。

:::note

* 工厂 AT 固件已设置为 RDP（读保护）等级 1，开发者需要先使用 STM32Cube Programmer 移除 RDP。注意，将 RDP 回归到等级 0 会导致闪存内存全部擦除，工厂 AT 固件将无法恢复。

* Wio-E5 模块上的 "PB13/SPI_SCK/BOOT" 引脚只是一个普通的 GPIO，而不是 MCU 的 "BOOT0" 引脚。此 "PB13/SPI_SCK/BOOT" 引脚在工厂 AT 固件的引导加载程序中用于决定跳转到 APP 或停留在引导加载程序（用于 DFU）。真正的 "BOOT0" 引脚未引出到模块，因此用户在开发低功耗应用时需要特别注意。
...
<tbody>
  <tr>
    <td rowspan="2">结构</td>
    <td>尺寸</td>
    <td colspan="4">12(宽)*12(长)*2.5(高)</td>
    <td>毫米</td>
  </tr>
  <tr>
    <td>封装</td>
    <td colspan="4">28引脚，SMT</td>
    <td></td>
  </tr>
  <tr>
    <td rowspan="18">电气特性</td>
    <td>电源电压</td>
    <td colspan="4">3.3V类型</td>
    <td>V</td>
  </tr>
  <tr>
    <td>休眠电流</td>
  ...
  <tr>
    ...
    <td colspan="4">50mA @10dBm，434MHz类型</td>
    <td rowspan="3">mA</td>
  </tr>
  <tr>
    <td colspan="4">111mA @22dBm，470MHz类型</td>
  </tr>
  <tr>
    <td colspan="4">111mA @22dBm，868MHz类型</td>
  </tr>
  <tr>
    <td rowspan="3">工作电流（接收器+MCU）</td>
    <td colspan="4">6.7mA @BW125kHz，868MHz类型</td>
    <td rowspan="3">mA</td>
  </tr>
  <tr>
    <td colspan="4">6.7mA @BW125kHz，434MHz类型</td>
  </tr>
  <tr>
    <td colspan="4">6.7mA @BW125kHz，470MHz类型</td>
  </tr>
  <tr>
    <td rowspan="3">输出功率</td>
    <td colspan="4">10dBm最大值 @434MHz</td>
    <td rowspan="3">dBm</td>
  </tr>
  <tr>
    <td colspan="4">22dBm最大值 @470MHz</td>
  </tr>
  <tr>
    <td colspan="4">22dBm最大值 @868MHz</td>
  </tr>
  <tr>
    <td rowspan="5">灵敏度</td>
  ...
  <tr>
    <td rowspan="2">谐波</td>
    <td colspan="4">&lt; -36dBm，低于1GHz</td>
    <td>dBm</td>
  </tr>
  <tr>
    <td colspan="4">&lt; -40dBm，高于1GHz</td>
    <td>dBm</td>
  </tr>
  <tr>
    <td rowspan="6">接口</td>
  ...
  <tr>
    <td>ADC</td>
    <td colspan="4">1个ADC输入，包括1个引脚，12位1Msps</td>
    <td></td>
  </tr>
  <tr>
    <td>NRST</td>
...
* <p><a href="https://files.seeedstudio.com/products/317990687/res/LoRa-E5%20AT%20Command%20Specification_V1.0%20.pdf">Wio-E5 AT命令规范</a></p>
...
* <p><a href="https://my.st.com/content/my_st_com/en/products/embedded-software/mcu-mpu-embedded-software/stm32-embedded-software/stm32cube-mcu-mpu-packages/stm32cubewl.license=1608693595598.product=STM32CubeWL.version=1.0.0.html#overview" target="_blank">STM32Cube MCU包，用于STM32WL系列</a></p>
...
* **步骤 1.** 使用 Type-C 数据线将 Wio-E5 开发板连接到电脑
...
* **步骤 9.** 根据你的地区选择 **Frequency plan**。同时确保使用与将连接此设备的网关相同的频率。选择 **MAC V1.0.2** 作为 **LoRaWAN® version**，选择 **PHY V1.0.2 REV B** 作为 **Regional Parameters version**。这些设置与 Wio-E5 的 LoRaWAN® 协议栈一致。
...
* **步骤 11.** 将上述信息复制并粘贴到 **DevEUI**、**AppEUI** 和 **AppKey** 字段中。当我们填写 **DevEUI** 时，**End device ID** 字段会自动填充。最后点击 **Register end device**。
...
* **步骤 12.** 在 TTN 控制台中注册您的 LoRaWAN® 网关。请参考[此处](https://wiki.seeedstudio.com/cn/The-Things-Indoor-Gateway/#step-2-gateway-registration-on-ttn-console)显示的说明。
...
**注意：** 我们现在已更新库以支持 STM32WL 系列的最新版本 STM32Cube MCU Package v1.1.0。

:::note
请先阅读 [Erase Factory AT Firmware](https://wiki.seeedstudio.com/cn/LoRa_E5_mini/#21-erase-factory-at-firmware) 部分，因为在使用 SDK 编程之前需要擦除工厂 AT 固件。擦除工厂 AT 固件后将无法恢复。
:::
...
* **步骤 5.** 进入 `C/C++ Build > Settings > MCU Post build outputs`，勾选 **Convert to Intel Hex file (-O ihex)**，然后点击 **Apply and Close**。
...
* **步骤 6.** 点击 **Build 'Debug'**，代码应能成功编译且无任何错误。
...
* **步骤 7.** 请按照[此指南](https://wiki.seeedstudio.com/cn/LoRa_E5_mini/#13-connect-and-send-data-to-the-things-network)设置您的 TTN 应用，获取您的 **Application EUI**，并将其复制到 `LoRaWAN/App/se-identity.h` 文件中的宏定义 `LORAWAN_JOIN_EUI`。例如，这里的 Application EUI 是 `80 00 00 00 00 00 00 0x07`：
...
* **步骤 8.** 同样，您可以通过设置 `LoRaWAN/App/se-identity.h` 文件中的宏定义 `LORAWAN_DEVICE_EUI` 和 `LORAWAN_NWK_KEY` 来修改 **Device EUI** 和 **Application Key**。确保 `LORAWAN_DEVICE_EUI` 和 `LORAWAN_NWK_KEY` 与 TTN 控制台中的 `Device EUI` 和 `App Key` 一致。
...
* **步骤 9.** 默认的 LoRaWAN® 区域是 `EU868`，您可以通过设置 `LoRaWAN/App/lora_app.h` 文件中的宏定义 `ACTIVE_REGION` 来修改它。
...
* **步骤 10.** 完成上述修改后，**重新构建**示例并将其烧录到您的 Wio-E5 中。打开 `STM32CubeProgrammer`，将 ST-LINK 连接到您的电脑，按住设备的 `RESET 按钮`，然后点击 `Connect` 并释放 `RESET 按钮`：
...
* **步骤 13.** 如果您的 LoRaWAN® 网关和 TTN 已设置完成，Wio-E5 在复位后将成功加入网络！每 30 秒会向 TTN 发送一个确认的 LoRaWAN® 数据包。如果加入成功，串口监视器（这里使用 Arduino 串口监视器）将打印以下日志：
...
* 恭喜！现在您已将 Wio-E5 连接到 LoRaWAN® 网络！您现在可以继续开发更多有趣的 LoRaWAN® 终端节点应用程序！

**注意：** Wio-E5 仅支持高功率输出模式，因此您不能在 `radio_board_if.h` 文件中使用以下宏定义：
...
尽管 **RBI_CONF_RFO** 在 `radio_board_if.h` 中被定义为 **RBI_CONF_RFO_LP_HP**，但它不会被使用，因为定义了 **USE_BSP_DRIVER**，并且 **BSP_RADIO_GetTxConfig()** 函数返回的是 **RADIO_CONF_RFO_HP**。

#### FreeRTOS LoRaWAN®

此应用程序还将连接 Wio-E5 mini/Wio-E5 开发套件到 TTN（The Things Network），并在连接到 LoRaWAN® 网关后发送数据。与之前的 LoRaWAN End Node 应用程序的区别在于，前者运行在裸机上，而此应用程序运行在 FreeRTOS 下。
...
* **步骤 4.** 参考之前 **LoRaWAN® End Node** 应用程序的 **步骤 4 - 步骤 13**，将 Wio-E5 mini/Wio-E5 开发套件连接到 TTN！

#### FreeRTOS LoRaWAN® AT

此应用程序还将连接 Wio-E5 mini/Wio-E5 开发套件到 TTN（The Things Network），并在连接到 LoRaWAN® 网关后发送数据。与之前的 FreeRTOS LoRaWAN® 应用程序的区别在于，您可以使用 AT 命令。
...
* **步骤 7.** 如果您仍然想更改 **Device EUI**、**Application EUI**、**Application KEY** 和 **LoRawan Region**，可以使用 AT 命令更改。然而，这些参数已经在本示例中的 **se-identity.h** 和 **lora_app.h** 中设置好了。
...
* **步骤 8.** 确保读取保护设置为 `AA`，如果显示为 `BB`，请选择 `AA` 并点击 `Apply`：
...
* **步骤 10.** 将 Wio-E5 mini/Wio-E5 开发套件通过电流表连接到电脑。您会注意到板上的红色 LED 每秒闪烁一次，板子会在正常和低功耗状态之间切换（电流表上的电流在低功耗状态下会下降 1 秒，然后在正常工作状态下恢复 1 秒）。
...
* **步骤 4.** 参考之前 **FreeRTOS 低功耗** 应用程序中的 **步骤 4 - 步骤 10**，您将在电流表上看到相同的输出结果！
