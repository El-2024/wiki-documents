---
description: 快速入门 Wio Tracker 1110
title: 快速入门 Wio Tracker 1110
keywords:
- Tracker
- Wio
...
在本教程中，我们将指导用户通过工厂固件快速设置 Wio Tracker 1110 开发板，将其连接到 SenseCAP 云并检查数据。
...
通过 `扫描` 搜索设备，当您在列表中找到与自己设备相同的 `S/N 码` 项目时，点击进入。
...
<table>
  <tbody>
    ...
    <tr>
      ...
      <td>默认平台。
必须与 SenseCAP 网关一起使用。SenseCAP 构建了一个专有的 TTN 服务器，当与 SenseCAP 网关配对时，传感器可以开箱即用。<br />
    ...
    <tr>
      ...
      <td>当有 Helium 网络覆盖时，数据可以通过 Helium 上传。设备运行在 SenseCAP 的私有 Helium 控制台上。用户无需在 Helium 控制台上创建设备，使用 SenseCAP Mate App 和 Portal 即可开箱即用。<br />
...
| 参数 | 描述 |
| - | - |
| 位置上传间隔（分钟） | 定时上传位置信息。<br/>默认 5 分钟。<br/>频率越高，功耗越高。 |
| 传感器上传间隔（分钟） | 定时上传传感器数据。<br/>默认 5 分钟。<br/>频率越高，功耗越高。 |
...
当设置中的所有信息都符合您的要求时，您可以点击 `发送` 将其传输到 **Tracker Wio 1110**。
...
启动开发板，并确保附近有网络覆盖。当开发板成功连接到网络时，它将在 `设备` 页面显示 `在线`。
...
如果您所在的区域信号较差，只需安装一个 LoRaWAN 网关即可——这就像设置 Wi-Fi 路由器一样简单。LoRa 的魅力在于其灵活性和网络扩展的简便性。这种可靠的解决方案还具有成本效益。例如，[SenseCAP M2 室内 LoRaWAN 网关](https://www.seeedstudio.com/SenseCAP-Multi-Platform-LoRaWAN-Indoor-Gateway-SX1302-EU868-p-5471.html)，可以将您的覆盖范围扩展至 10 公里！
...
:::tip
请将开发板放置在窗户旁边或无遮挡的地方，以便获得更好的 GPS 信号，从而使开发板能够成功获取位置信息。
:::
...
如果您已通过 APP 创建了账户，可以直接登录。
导航到您的设备页面，点击 `Data` 查看数据。
...
1、当开发板被摇动引发振动时，会触发**三轴加速度传感器**立即采集并上传数据。
...
当设备被摇动时，SenseCAP Mate 应用界面将显示数据包。
...
除了板载传感器外，Wio Tracker 1110 开发板还提供了无限的可能性。在出厂固件中，以下 Grove 传感器也可以接入此开发板并被自动识别。
...
将 Grove 传感器连接到 Wio Tracker 开发板上的对应 Grove 接口。

您还可以使用 [Grove-I2C Hub](https://www.seeedstudio.com/Grove-I2C-Hub.html) 将多个 I2C 传感器连接到开发板。
...
双击 `Reset` 按钮，您的电脑上应该会出现一个名为 `WM1110_BOOT` 的驱动器。
...
将 `.uf2` 文件拖到该驱动器中。下载过程会自动运行，完成后驱动器会自动退出。


## SenseCAP API

SenseCAP API 用于帮助用户管理物联网设备和数据。它包含三种类型的 API 方法：HTTP 协议、MQTT 协议和 Websocket 协议。
...
请查阅 [API 用户指南](https://sensecap-docs.seeed.cc/) 了解更多详情。
