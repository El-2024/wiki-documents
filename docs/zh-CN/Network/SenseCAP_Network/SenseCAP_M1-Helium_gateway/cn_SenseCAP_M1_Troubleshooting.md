...
**注意**: 每个设备都有一个与之注册的唯一 'config.json' 文件，请不要与他人共享您的 'config.json' 文件，也不要使用他人的文件。
...
**谨慎操作**：以下步骤仅在您已收到我们技术支持团队的建议时使用。如果您未收到建议，请勿执行这些步骤。
...
1. 如果您在通过以太网或 Wi-Fi 连接后立即注册热点，可能会出现此错误，因为 OTA（空中更新）正在运行。
