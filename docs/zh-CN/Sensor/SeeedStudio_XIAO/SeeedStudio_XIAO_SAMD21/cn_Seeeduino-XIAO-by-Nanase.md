...
# Seeed Studio XIAO SAMD21 入门指南 By Nanase

这是一份由 [@nanase_coder](https://twitter.com/nanase_coder) 编写的文档。（由 Seeed 从原始日文文档翻译：[コインサイズ Arduino互換機 Seeed Studio XIAO SAMD21 を使ってみた](https://qiita.com/nanase/items/0fed598975c49b1d707e#spi-microsd%E3%82%AB%E3%83%BC%E3%83%89)）。感谢 Nanase 与我们分享这份文档！
...
板子背面没有元件，所有引脚都有半孔设计，便于焊接到其他板子上。
...
如您所见，这是一个 SAMD 类型的 Arduino，类似于 Arduino MKR 系列，因此从技术上讲，为它们编写的任何库都可以在 Seeed Studio XIAO SAMD21 上使用。另一方面，由于它与 Arduino Uno 等 ATmega 类型的 Arduino 不同，依赖于 ATmega 某些特定寄存器的库无法使用。
...
查看[这里](https://wiki.seeedstudio.com/cn/Seeeduino-XIAO/#software)并了解如何将 Seeed Studio XIAO SAMD21 添加到您的 Arduino IDE 中。
...
Seeed Studio XIAO SAMD21 没有重置按钮。相反，它有一个重置焊盘。
短接这个重置焊盘并接地来重置您的开发板。
...
有时程序可能会崩溃或无法上传代码。您可以重置开发板两次进入引导加载程序模式。在此模式下，LED 缓慢闪烁，Seeed Studio XIAO SAMD21 被识别为 USB 存储设备。串口与正常模式分离，始终处于代码写入模式，不会运行开发板上的先前程序。

要从引导加载程序模式返回到正常模式，请上传一个代码或再次快速重置两次。

如果当 Seeed Studio XIAO SAMD21 未被 PC 识别为 USB 设备时进入引导加载程序模式，LED 将快速闪烁。

## LCD

就像原始的 Arduino 一样，从示例代码中选择 Basics> Blink 并上传。
...
官方网站将另外两个内置 LED 描述为`用于串口下载的两个 LED`。但是，查看原理图，这些 RX 和 TX LED 没有连接到物理引脚。

如果您查看[这里的 USBCore.cpp](https://github.com/Seeed-Studio/ArduinoCore-samd/blob/1.7.2/cores/arduino/USB/USBCore.cpp#L622-L627)，您可以看到每次发生串口 USB 传输/接收时，它们都会通过 `digitalWrite` 点亮，这意味着这两个 LED 是可编程的。
...
具体的引脚号码在 `variant.h / variant.cpp` 中定义，就像 SAMD Arduino 一样，在 Seeeduino XIAO 的情况下，它们被分配给 11 和 12，如下所示。
...
下面是一个闪烁三个 LED 的代码。RX 和 TX 的 LED 是蓝色的。
...
如果一次发送 1 字节，只能获得 0.11 Mbps（14.53 KB / s），但如果发送 64 字节，速度会显著提高到 6.30 Mbps（805.86 KB / s）。您可以推断内部缓冲区的大小是 64 字节。

如上所述，LED 在串行通信中会闪烁，但由于这个原因几乎没有速度下降。

### SPI（MicroSD 卡）

Seeed Studio XIAO SAMD21 的逻辑电平为 3.3V。换句话说，可以通过 SPI 处理 microSD 卡而无需电平转换器。
SPI 的某些功能与 ATmega 类型的 Arduino 不同，请查看[这里](https://qiita.com/nanase/items/406a8a848d1b259d3af4)获取更多信息。

这里我们将使用秋月电子的 [micro SD 卡插槽 DIP 套件](http://akizukidenshi.com/catalog/g/gK-05488/)读取 microSD 卡。

我们在这里使用 Arduino 示例代码，但 `SD.begin (cs_pin)` 的参数指定为 `SS`。根据 variant.h，SS = 4，这与 D4 / A4 / SDA 引脚相同。当然，您可以指定其他引脚。
...
I2C 也可用。3.3V 设备可以直接连接而无需电平转换器。

这次，我们使用 BME280 来测量温度、湿度和大气压力。BME280 在 3.3V 下工作，因此可以在不使用电平转换器的情况下连接。查看[这里](https://qiita.com/nanase/items/f34e03c29410add9c4d0)获取 Arduino 和 BME280 之间连接的详细说明。
...
这次，我们将 Seeed Studio XIAO SAMD21 连接到 GPS 接收器套件，并从 PC 获取 NMEA 信息。使用 Xiao 时这是一个非常简单的工作，它只是 GPS 套件和 PC 串口之间的桥梁。
...
这次我们使用 GPSFox 来浏览 NMEA 信息。可以轻松测量坐标。
...
作为 SAMD 类型 Arduino 的功能之一，您可以在 Xiao 上使用 DMA。查看[这里](https://qiita.com/nanase/items/406a8a848d1b259d3af4#dma)了解更多关于 DMA 的信息。

### 单周期 IOBUS

Cortex M0+ 具有一个称为[单周期 IOBUS](https://microchipdeveloper.com/32arm:samd21-iobus-overview)的功能，可以在一个时钟周期内操作GPIO输出。写入特定寄存器可以反转逻辑、禁用引脚或更改引脚驱动电流。

### DigitalWrite

您可以使用 digitalWrite 创建脉冲，这是一种适用于任何 Arduino 板的方法 - 只是重复开销。
...
您也可以通过直接操作寄存器而不使用 `digitalWrite` 来创建脉冲。
...
我们使用[这里](https://lab.sasapea.mydns.jp/2020/03/16/seeeduino-xiao/)介绍的 `IOBUS.h`。
