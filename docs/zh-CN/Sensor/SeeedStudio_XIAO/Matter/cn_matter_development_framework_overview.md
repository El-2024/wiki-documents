---
description: 以代码之光为例，介绍Matter的开发框架。
title: Matter开发框架概述
keywords:
- matter
- XIAO
...
:::tip
本文是Seeed Studio XIAO ESP32开发Matter系列的第四篇教程。如果您还没有阅读之前的教程，我们建议您先阅读它们，以检查您的设备是否已按要求配置。

- **[在XIAO上使用Espressif ESP-IDF进行开发](https://wiki.seeedstudio.com/cn/xiao_idf)**
...
通过我们的综合教程，踏上进入Matter开发世界的启发性旅程。通过经典灯光示例的视角，我们将阐明构成Matter开发基础的基本概念和知识。从集群和属性到命令等等，本教程将为您提供自信地导航Matter框架的工具。准备好开启您的Matter开发技能，让我们深入探索连接设备的精彩世界！
...
4. **[设置端点属性](#sets-the-attribute-of-the-endpoint)**
5. **[创建端点和自动匹配集群](#create-endpoint--auto-match-cluster)**
6. **[首次设置Matter设备的默认值](#setting-up-the-matter-device-for-the-first-time-with-default-values)**
7. **[数据更新和延迟持久化](#data-updates-and-deferred-persistence)**

在本节中，我们将重点关注ESP-Matter中提供的[light](https://github.com/espressif/esp-matter/tree/main/examples/light)示例，该示例详细描述了Matter开发框架中集群、属性和命令的基本概念。通过阅读本文，您将能够更好地理解Matter开发框架的结构和认知。
...
**esp-matter**：这是整个Matter开发框架的根目录。

**components**：此目录包含各种组件，是Matter框架的核心。
   - esp_matter：这是ESP32上Matter协议栈的实现，包括数据模型、应用层逻辑等。
   - esp_matter_bridge：此组件负责将ESP设备桥接到其他非ESP设备，实现互操作性。
   - esp_matter_console：这是一个基于REPL的交互式控制台，用于调试和控制Matter设备。
   - esp_matter_controller：此组件实现Matter控制器的功能，能够控制其他Matter设备。
   - esp_matter_rainmaker：此组件与Espressif的RainMaker云平台集成，实现云控制。
   - esp_matter_thread_br：此组件实现Thread边界路由器功能，用于创建Thread网络。

**connectedhomeip**：这是Matter协议栈的上游开源项目，ESP Matter从中同步代码。
...
**docs**：这是存储ESP-Matter开发文档和API参考手册的地方。

**examples**：各种示例代码，演示如何使用Matter框架进行开发。

**tools**：包含各种开发工具脚本。
   - mfg_tool：用于生成制造商证书的工具。

**CMakeLists.txt**：CMake构建脚本，定义项目的编译规则。

**RELEASE_NOTES.md**：发布说明，记录每个版本的更改。

**export.sh**：导出脚本，用于导出Matter相关的环境变量。

**install.sh**：安装脚本，用于安装Matter开发所需的依赖项和工具链。

**requirements.txt**：Python依赖项列表，指定运行Matter开发框架所需的Python库。

这种目录结构反映了Matter开发框架的模块化设计理念。核心协议栈、硬件抽象、应用组件、辅助工具等各部分各司其职，同时有机结合，为开发者提供了完整的Matter开发环境。

以**examples/light**为例，ESP-Matter提供的示例结构如下：
...
- **main**：此子目录包含主应用程序代码和配置文件。
   - CMakeLists.txt：主应用程序的CMake构建脚本。
   - app_driver.cpp：灯光应用程序的驱动程序代码。
   - app_main.cpp：灯光应用程序的主入口点。
   - app_priv.h：包含灯光应用程序私有声明的头文件。
   - idf_components.yml：灯光应用程序中使用的ESP-IDF组件的配置文件。

- **CMakeLists.txt**: 灯光示例的顶级 CMake 构建脚本。

- **README.md**: 为灯光示例提供信息和说明的自述文件。
...
`app_reset_button_register(button_handle);`: 这行代码为特定功能注册按钮，以处理重置操作。
...
Matter 设备配置主线代码的下一步是创建 Matter 节点。代码如下：
...
2. `light_config.on_off.lighting.start_up_on_off = nullptr;`
   ...
   - 当设备重启或电源循环时，如果此值为 `nullptr`，意味着使用最后的开/关状态。
...
4. `light_config.level_control.lighting.start_up_current_level = DEFAULT_BRIGHTNESS;`
   ...
   - 当设备重启或电源循环时，如果此值为非 `nullptr`，意味着使用指定的亮度级别。
...
7. `light_config.color_control.color_temperature.startup_color_temperature_mireds = nullptr;`
   - 将端点的启动色温设置为 `nullptr`。
   - 当设备重启或电源循环时，如果此值为 `nullptr`，意味着使用最后的色温设置。
   - 如果设置为非 `nullptr` 值，则表示使用指定的色温值。

Matter 中的属性就像设备的属性或特征。它们存储有关设备状态的信息，例如设备是开还是关、亮度级别或色温。这些属性被组织成称为集群的组，这些集群与设备的特定功能相关。

属性使不同的设备和应用程序更容易通信并无缝协作。通过拥有表示和访问设备属性的标准方式，Matter 简化了智能家居系统的开发，并确保来自各种品牌的设备能够高效地相互交互。
...
在上面，我们首先提到了在 Matter 中很重要的两个术语，端点和簇。

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/xiaoc6-matter/35.png" style={{width:500, height:'auto'}}/></div>

**端点 [设备类型]**：
端点是 Matter 节点内特定功能或服务的逻辑表示。它封装了与特定设备类型相关的一组能力和行为。一个 Matter 节点可以有多个端点，每个端点代表不同的设备类型。设备类型定义了端点的特定特征和功能。Matter 定义了一组标准设备类型，如灯泡、恒温器、门锁等。每种设备类型都有唯一的标识符和与之关联的预定义簇、属性和命令集。

- 端点是 Matter 节点内的逻辑组件，代表设备的特定功能或服务。
- 就像房子里的房间一样，每个端点都有自己专门的用途，比如卧室、厨房或客厅。
- 每个端点都与特定的设备类型相关联，如灯泡、恒温器或门锁。
- 一个 Matter 节点可以有多个端点，每个端点代表不同的设备类型和功能。

**簇**：
簇是端点内相关属性和命令的逻辑分组。它们代表设备的特定功能或特性。簇提供了一种组织和分类端点能力的方式。例如，"开/关簇"包含与设备开关相关的属性和命令，而"电平控制簇"处理控制设备亮度或电平的功能。

- 簇是端点内的逻辑分组，包含相关的属性和命令。
- 它就像房间里的家具或设备，如灯具、电视或空调，每个都有自己的属性和操作。
- 每个簇代表设备的特定功能或特性。
- 例如，"开/关簇"包含与设备开关状态相关的属性和命令，而"电平控制簇"包含用于调节设备亮度或电平的属性和命令。
- 一个端点可以有多个簇，每个簇负责不同的功能。

总结来说，一个 Matter 节点就像一座房子，包含多个端点（房间）。每个端点就像一个房间，代表设备的特定功能或服务。簇就像每个房间里的家具或设备，包含用于控制和交互的相关属性和命令。

这种分层组织允许设备清楚地描述其功能和特征，使应用程序和其他设备更容易与它们交互和控制。通过标准化设备类型、簇、属性和命令，Matter 实现了不同制造商设备之间的互操作性和兼容性。

在代码中，设置属性后，最终通过以下代码片段创建端点。它会自动匹配设置属性的簇。
...
如何实现自动匹配簇？让我们以设置属性的代码片段为例。

```cpp
light_config.level_control.lighting.start_up_current_level = DEFAULT_BRIGHTNESS;
```

`light_config.level_control` 是在端点（esp_matter_endpoint.h）中定义的属性。而 `light_config.level_control.lighting` 是在簇（esp_matter_cluster）中定义的属性。通过这种设置，系统可以自动匹配属性对应的簇，无需开发者手动设置。

## 首次使用默认值设置 Matter 设备

一旦上述属性、集群和端点已经配置完成，我们就可以开始启动 Matter 设备。启动的步骤和方法如下。
...
如您所见，设置默认值的函数是 `app_driver_light_set_defaults()`，我们需要传入一个端点 ID 作为参数。我们需要关注如何获取某个集群、某个属性的值，以及如何设置默认的集群、属性值。秘诀在 `app_driver.cpp` 中展示。
...
1. 获取集群：
   - 要获取集群，首先需要使用 `endpoint::get(node, endpoint_id)` 函数获取端点的指针，其中 `node` 是节点的指针，`endpoint_id` 是端点的 ID。
   - 获得端点指针后，可以使用 `cluster::get(endpoint, LevelControl::Id)` 函数获取所需集群的指针，指定端点和集群 ID（在这种情况下是 `LevelControl::Id`）。

2. 获取特定属性：
   - 获得集群指针后，可以使用 `attribute::get(cluster, LevelControl::Attributes::CurrentLevel::Id)` 函数获取该集群内特定属性的指针。
   - 在这个例子中，我们从 `LevelControl` 集群中检索 `CurrentLevel` 属性。

3. 检索属性的值：
   - 要检索属性的当前值，需要声明一个 `esp_matter_attr_val_t` 类型的变量来存储属性值。
   - 在代码片段中，变量 `val` 用 `esp_matter_invalid(NULL)` 初始化。
   - 然后，可以使用 `attribute::get_val(attribute, &val)` 函数获取属性的当前值并将其存储在 `val` 变量中。

4. 设置灯的亮度：
   - 要设置灯的亮度，需要有与端点关联的 LED 指示器的句柄。
   - 在代码片段中，LED 指示器句柄通过将私有数据指针（`priv_data`）转换为适当的类型（`led_indicator_handle_t`）来获得。
...
在 `app_driver.cpp` 的代码中，属性值通过使用 `app_driver_attribute_update()` 函数进行更新。
...
5. 基于 `cluster_id` 和 `attribute_id`，函数调用相应的设置函数来更新属性值：
...
```
   ...
   ``window_covering_device`` ``config_t`` 结构包含一个构造函数，允许指定与默认产品类型（"卷帘"）不同的最终产品类型。
   一旦 ``config_t`` 实例被实例化，其最终产品类型就无法修改。
...
```
   ...
   ``window_covering`` ``config_t`` 结构包含一个构造函数，允许指定与默认产品类型（"卷帘"）不同的最终产品类型。
   一旦 ``config_t`` 实例被实例化，其最终产品类型就无法修改。
