...
在设置 Watcher 与 Node-RED 协作后，下一步是为 Discord 准备来自 Watcher 的数据。双击功能节点以适当格式化数据。
...
需要 Discord 账户才能继续。如果您还没有账户，请[**点击这里**](https://discord.com)并完成创建账户的步骤。

如果您已经有账户，请继续到[**步骤 4**](#step-4-create-development-application)。

如果您已经有一个想要在 Node-RED 中配置的 Discord 机器人，请跳到[**第三部分**](#part-3-integrate-discord-with-node-red)。

### 步骤 4：创建开发应用程序

一旦您设置好 Discord 账户并登录，请前往[**Discord 开发者门户**](https://discord.com/developers/applications)并创建一个新应用程序。
...
2. 在**特权网关意图**部分，您将看到几个选项。在本教程中，我们只需要启用**消息内容意图**。这允许您的机器人读取消息内容。切换**消息内容意图**旁边的开关以启用它。启用后，开关将变为蓝色。
...
请注意，重置您的令牌将导致您的机器人停止工作，直到您在机器人代码中更新令牌。在继续重置之前，请确保您已准备好更新机器人的配置。

您可能会被提示进行多因素身份验证。如果是这样，请按要求完成此步骤，然后再继续。
...
:::warning
切勿公开分享您的机器人令牌。任何拥有您令牌的人都可以控制您的机器人。如果您意外泄露了令牌，请立即在此页面上重置它。
:::

### 步骤 6. 配置 OAuth2 设置

现在，切换到**设置**下的 **OAuth2** 选项卡，向下滚动直到看到 **OAuth2 URL Generator**。在这里，我们将勾选 **bot** 范围，因为我们正在创建一个机器人应用程序。
...
选择机器人范围后，将出现一个新框，允许您选择**机器人权限**。在本教程中，我们将为机器人授予**管理员**权限，这为机器人提供了对所有功能的访问权限。
...
:::warning
向机器人授予管理员权限将使其完全访问所有服务器功能。在生产环境中，建议遵循最小权限原则，仅授予机器人执行其预期功能所需的特定权限。
:::
...
系统将提示您选择要添加机器人的服务器。从下拉菜单中选择适当的服务器，然后点击**继续**。
...
在下一个屏幕上，您将看到您授予机器人的权限的完整列表。这些权限基于您在[**步骤 6**](#step-6-configure-oauth2-settings)的 OAuth2 URL Generator 中选择的内容。仔细查看每个权限，然后点击**授权**。
...
与[**步骤 5**](#step-5-configure-bot-settings)类似，您可能会再次被提示进行多因素身份验证。如果是这样，请按要求完成此步骤，然后再继续。

成功授权后，您将看到一条确认消息，表明机器人已添加到您选择的 Discord 服务器。现在，返回您的 Discord 服务器继续操作。
...
在用户设置侧边栏中，向下滚动到**应用设置**部分，点击**高级**。确保**开发者模式**切换开关已打开。
...
关闭用户设置并返回您的 Discord 服务器。右键点击您要使用的频道（在本教程中，我们使用**general**频道）。在出现的上下文菜单底部，点击**复制频道 ID**。保存此频道 ID 以在您的 Node-RED 流程中使用。
...
在调色板管理窗口中切换到 **Install** 选项卡。在搜索栏中输入 `discord` 来查找模块。然后，点击 **node-red-contrib-discord-advanced** 条目旁边的 **install** 按钮。
...
如果您想更深入地了解该模块的工作原理以及如何最大化其潜力，我们还建议您查看[**节点文档**](https://github.com/Markoudstaal/node-red-contrib-discord-advanced/blob/main/README.md)和相关的[**Github 仓库**](https://github.com/Markoudstaal/node-red-contrib-discord-advanced)。

:::note
在编写本教程时（2024年8月），推荐使用 **node-red-contrib-discord-advanced** 模块，因为它具有当前的兼容性和可靠性。但是，Node-RED 模块的可用性和功能可能会随时间而变化。建议查看 Node-RED 库或 GitHub 仓库中最新的用户反馈和兼容性说明。如果您在使用此模块时遇到问题，请考虑探索 Node-RED 库中列出的其他 Discord 模块以获得更合适的选择。
:::

### 步骤 10. 配置 Discord 节点

安装 Discord 模块后，将 **discordMessageManager** 节点拖拽到您的 Node-RED 流程中，并将其连接到函数节点
...
在 **Channel** 字段中，输入您在[步骤 8](#step-8-obtain-channel-id)中获得的频道 ID。然后，点击 **Done** 关闭配置面板。
...
:::note
对于具有多个频道和触发条件的更复杂设置，您可以将 Channel 字段留空，并使用 `msg.channel` 动态设置频道。但是，对于这个基础教程，我们将使用静态频道 ID。
:::

### 步骤 11. 部署

最后，点击 Node-RED 界面右上角的 **Deploy** 按钮。部署完成后，您的 Watcher 将向 Node-RED 发送消息，然后 Node-RED 会将这些消息转发到您的 Discord 频道。
...
恭喜！您已成功配置您的 Watcher 通过 Node-RED 向 Discord 发送消息。此设置确保您在 Watcher 被触发时能在 Discord 频道中收到实时通知。通过这样做，您为开发项目解锁了大量令人兴奋的机会。我们热切期待您接下来将创造的创新解决方案！
...
- 复制生成的 URL 并在新标签页中打开，以使用这些权限将机器人邀请到您的服务器。
