...
为您的配置命名，选择 Create and Verify，然后 Finish。
...
#
#要使用的 MQTT 代理 URL。格式为 scheme://host:port，schema 可以是 tcp、ssl 或 ws。
...
#
#仅当您的负载类型为字符串时才需要，因为 Telegraf 默认不转发此类型的数据
...
接下来，您需要从 Tokens 选项卡复制之前生成的令牌，并将其导出为环境变量供 InfluxDB 输出插件使用，或者您可以简单地在配置文件中直接将其作为令牌值传递。您可以在终端中使用以下命令设置环境变量：
...
本节向您展示如何使用 HTTP Listener v2 插件配置 Telegraf 代理，以及如何在 The Things Stack 上创建相应的 Webhook 集成。

按照 InfluxDB Cloud 2.0 设置中描述的方式更新您之前下载的 Telegraf 配置，通过添加以下行并根据您的设置进行修改：
...
选择您的浏览器类型，在过滤器窗口的下拉菜单中选择 **_measurement**，并勾选 **mqtt_consumer** 框。然后您可以选择要监控的主题和参数。
...
您还可以点击右上角的 **SAVE AS** 将此浏览器保存为仪表板单元格。
