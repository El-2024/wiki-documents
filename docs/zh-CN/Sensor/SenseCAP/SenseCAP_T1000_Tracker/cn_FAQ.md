...
* 当追踪器在室内时，可能无法获得 GPS 位置。GNSS 定位需要开阔的室外环境。如果您在室内，由于信号微弱，GPS 位置可能会超时。请确保将设备放置在室外以获得 GPS 精度。
...
### 为什么 Wi-Fi 或蓝牙位置不在 SenseCAP 应用地图上显示？
...
长按按钮3秒，绿灯慢闪，然后按一次按钮，绿灯呼吸，设备将尝试加入LoRaWAN网络。

### 强制重启

长按按钮，然后连接充电线，连接后松开按钮，绿灯呼吸，设备将被强制重启。
...
详细的电池寿命计算，请参考[电池寿命计算表](https://files.seeedstudio.com/products/SenseCAP/SenseCAP_Tracker/Trcaker_Battery_%20Life_Calculation_T1000_AB.xlsx)。
...
|充电中|LED每3秒闪烁一次。|
|充满电|LED常亮。|
|充电异常|当设备在0°C以下或45°C以上充电时，设备将进入充电保护状态，无法充电。<br/>LED将快速闪烁。|

:::caution 充电异常报警
如果充电时指示灯`快速闪烁`，可能是电源电压不足或接触不良。
...
为了节省电力，温度/光照传感器功能默认禁用，因此您需要先在SenseCAP Mate APP上启用它。
...
由于设备只能缓存约1,000条离线数据，如果您长时间处于没有LoRaWAN网络覆盖的地方，当离线数据达到限制时，一些旧数据将被新数据覆盖，因此您可能会"丢失"一些数据。
...
使用应用程序编程接口（API）是允许您的应用程序与其他应用程序或服务通信的常见方式，您可以将设备连接到SenseCAP云，然后使用SenseCAP API获取数据。

SenseCAP API供用户管理物联网设备和数据。它结合了三种类型的API方法：HTTP协议、MQTT协议和WebSocket协议。

查看[SenseCAP API](https://wiki.seeedstudio.com/cn/Cloud_Chain/SenseCAP_API/SenseCAP_API_Introduction/)了解更多详情。

* **LoRaWAN网络服务器**：

您可以使用[SenseCAP M2多平台网关](https://www.seeedstudio.com/SenseCAP-Multi-Platform-LoRaWAN-Indoor-Gateway-SX1302-EU868-p-5471.html)，然后您可以直接使用内置的LoRaWAN网络服务器进行集成。

查看[LNS配置](https://wiki.seeedstudio.com/cn/SenseCAP_m2_LNS_config)了解更多详情。