...
:::tip 提示
SenseCAP Mate 应用已正式更名为 `SenseCraft` 应用！
:::


SenseCraft 应用是一款功能强大的数据可视化和设备管理应用。
# 介绍与使用

## 功能

* 节省时间 - 提供扫描即用的体验，用户只需 4 步即可完成整个配置
...
SenseCraft 应用提供 iOS 和 Android 版本。
...
SenseCraft 支持设备配置和远程管理。要使用 SenseCAP 门户平台和其他功能，请注册一个账户。

:::tip 提示
请选择服务器位置为 `Global`。您也可以通过 <a href="http://sensecap.seeed.cc">SenseCAP 门户</a> 创建账户。
:::
...
AIGC 是一款由 AI 驱动的解决方案，帮助您充分利用传感器数据并释放其全部潜力。<br/>
通过将您的 SenseCAP 传感器连接到 SenseCAP 平台，您可以轻松收集和分析环境因素（如温度、湿度、光照和空气质量）的数据。同时，我们强大的 AI 算法会使用这些数据为您提供可操作的见解和建议，帮助您优化运营、降低成本并提高效率。
...
点击 [这里](https://wiki.seeedstudio.com/cn/How_to_Use_SenseCAP_AI_on_SenseCAP_Portal_and_SenseCAP_Mate_APP/) 查看更多详情。
...
1. 点击添加图标或“添加事件”按钮以创建事件警报。在“添加事件”页面中设置条件选项，并点击“添加”按钮选择设备。
...
2. 选择设备，选择测量类型，设置报警条件，选择大于或小于等条件，拖动进度条设置数值，点击下一步。
...
3. 在“添加事件”页面中，设置设备恢复正常后的操作，选择是否发送通知，点击保存，输入事件名称，点击提交以成功添加事件警报，返回事件列表。
...
4. 在设备页面，点击消息中心查看报警消息，显示设备警告、设备触发报警推送消息，以及系统通知的系统消息。
...
5. 点击报警消息，查看设备触发的条件项，推送的报警消息，点击查看报警详情。返回报警列表后状态变为已读，点击编辑按钮选择消息，可以进行标记已读、删除等操作。
