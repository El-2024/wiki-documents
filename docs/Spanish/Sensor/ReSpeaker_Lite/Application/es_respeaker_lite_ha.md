...
title: Sistema Asistente de Voz para Home Assitant
keywords:
- ESP32S3
- XIAO
- reSpeaker
...
Di "hola" al control sin manos y "adiós" a tocar pantallas con nuestro sistema de asistente por voz.<br/>

Imagina esto: estás cómodo en tu sofá, y con solo decir "ReSpeaker Lite", puedes encender las luces, subir la música o incluso preguntar por el clima, todo sin mover un dedo. Gracias al pequeño pero poderoso chip [Seeed XIAO ESP32S3](https://www.seeedstudio.com/XIAO-ESP32S3-p-5627.html), tu voz se conecta a Home Assistant, haciendo tu hogar más inteligente y tu vida más fácil.

En este capítulo usaremos el [Kit de Asistente de Voz ReSpeaker Lite](https://www.seeedstudio.com/ReSpeaker-Lite-Voice-Assistant-Kit-p-5929.html) para conectar un interruptor inteligente Sonoff y lograr el control de luces mediante voz.
...
- [Dispositivo con Home Assistant](https://wiki.seeedstudio.com/home_assistant_topic/)
- [Sonoff BASICR2](https://sonoff.tech/product/diy-smart-switches/basicr2/)

## Actualización del Firmware XMOS

Para una mejor experiencia de audio, es necesario actualizar el firmware XMOS a `respeaker_lite_i2s_dfu_firmware_48k_v1.0.9.bin`.

Descarga el firmware desde [aquí](https://github.com/respeaker/ReSpeaker_Lite/blob/master/xmos_firmwares/respeaker_lite_i2s_dfu_firmware_48k_v1.0.9.bin). Luego conecta el ReSpeaker Lite a tu computadora y ejecuta el siguiente comando:
...
## Primeros Pasos en Home Assistant

Accede a tu: [interfaz web de Home Assistant](http://homeassistant.local:8123/). 

Activa el `Modo Avanzado` para desbloquear funciones adicionales:
1. Haz clic en tu perfil.
2. Habilita la opción **Modo avanzado**.
...
1. Ve a [Ajustes > Complementos](https://my.home-assistant.io/redirect/supervisor)
...
2. En la sección oficial de ad-ons, instala los siguientes:
   - `ESPHome`
   - `Whisper`
   - `Piper`
   - `openWakeWord`
...
:::tip
Una vez finalizada la instalación, reinicia Home Assistant para aplicar los cambios.

Ve a: `Ajustes > Sistema > Reiniciar`.
...
Dirígete a `Ajustes > Dispositivos y Servicios`. Allí verás las integraciones detectadas automáticamente (Protocolo Wyoming).
...
Haz clic en `CONFIGURAR` y luego en `ENVIAR`.
...
Deberías ver 3 entidades creadas:
...
Utilizaremos [Sonoff BASICR2](https://sonoff.tech/product/diy-smart-switches/basicr2/) como ejemplo:

Configura el Sonoff según su manual.

En `HACS`, busca e instala `Sonoff LAN`.
...
Luego ve a `Ajustes > Dispositivos y Servicios > Añadir Integración > Sonoff`.
...
Ingresa tus credenciales de `eWeLink`.
...
Cuando esté conectado, verás una entidad
...
Selecciona tu palabra de activación preferida.
...
Gracias a @formatBCE, ya tenemos una integración genial con ESPHome. Para evitar soldaduras, se ha simplificado el archivo quitando la función del botón de usuario. Si deseas usar el botón, consulta: [github.com/formatBCE/Respeaker-Lite-ESPHome-integration](https://github.com/formatBCE/Respeaker-Lite-ESPHome-integration)
...
Haz clic en `SALTAR` para editar el archivo manualmente.
...
Luego haz clic en `Configurar` y pega el siguiente código:
...
Selecciona `Descarga Manual` -> `Formato moderno`.
...
Navega a [Web-ESPHome](https://web.esphome.io/), haz clic en `CONECTAR`, elige el puerto correcto y conéctalo.
...
Espera unos minutos para que termine la instalación. Cuando se complete con éxito, verás el siguiente mensaje:
...
Ve a `Ajustes > Dispositivos y Servicios`, verás `ESPHome` como una integración detectada. Haz clic en `CONFIGURAR`.
...
Haz clic en `ENVIAR`. ¡Todo está listo! Prueba activarlo diciendo `hey jarvis` y háblale.
...
* De **Smart Home Circle**:  
[Crear Asistente de Voz LOCAL con Home Assistant | Sin Soldaduras | Activación por Voz en el Dispositivo 🔥 | ReSpeaker Lite](https://www.youtube.com/watch?v=XjUeJh2Ok3o)
