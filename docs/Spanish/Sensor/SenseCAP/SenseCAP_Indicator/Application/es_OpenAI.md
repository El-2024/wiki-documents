...
# SenseCAP Indicator - ChatGPT - Desarrollo de Aplicaciones con DALL·E
...
Esta guía te proporcionará la información necesaria para organizar la demo de OpenAI en el SenseCAP Indicator, permitiendo la adición, eliminación y modificación rápida de programas, conforme al paquete de soporte de placa (BSP).

## Requisitos Previos

- Un [SenseCAP Indicator](https://www.seeedstudio.com/SenseCAP-Indicator-D1-p-5643.html)
- La herramienta [ESP-IDF](https://github.com/espressif/esp-idf) instalada en tu computadora

:::tip
Si deseas aprender cómo modificar la interfaz de usuario (UI), puedes consultar la guía: [Cómo Crear tu Propia UI](/SenseCAP_Indicator_How_to_Create_your_own_UI)

Si aún no tienes instalada la herramienta IDF, sigue las instrucciones de esta guía: [Cómo Flashear el Firmware Predeterminado](/SenseCAP_Indicator_How_To_Flash_The_Default_Firmware)
:::
...
El código principal relacionado con el flujo de inicio de OpenAI se muestra a continuación:
...
La aplicación SenseCAP Indicator x ChatGPT x DALL·E se desarrolla con arquitectura MVC (Modelo-Vista-Controlador). El flujo de trabajo del proyecto refleja claramente el uso de este patrón.

En esta arquitectura, los componentes están estructurados como:

- **Vista (View):** La Vista maneja señales disparadas por eventos y usa la biblioteca LVGL (Light and Versatile Graphics Library) para controlar la pantalla.

:::note
Para construir rápidamente la interfaz de usuario (UI), puedes utilizar [SquareLine Studio](https://squareline.io/), el cual también se utiliza en este proyecto. Se recomienda usar la versión 1.3.0 para una experiencia fluida.
:::

- **Modelo (Model):** El Modelo contiene el archivo `indicator_openai.c`, que incluye la función `indicator_openai_init()`. Al ejecutarse, esta función envía solicitudes a OpenAI, recibe las respuestas y las analiza para mostrarlas en pantalla a través de la Vista.

A continuación se muestran los diagramas de flujo clave del Modelo (una vez que se ha [guardado la API Key](/Sensor/SenseCAP/SenseCAP_Indicator/Set_An_API_Key)):
...
Para utilizar el servicio de OpenAI, es necesario implementar funciones que puedan enviar solicitudes, recibir respuestas y analizar la respuesta JSON. El siguiente fragmento de código ilustra la estructura necesaria:
...
La función `chat_request(...)` en `indicator_openai.c` se encarga de enviar la solicitud a la API de OpenAI, recibir la respuesta y analizar el JSON. Esta genera una solicitud HTTP con los datos del usuario y se comunica con el servidor utilizando `mbedtls_send_then_recv(...)`.

Para agregar un prompt a tu aplicación, puedes modificar la variable `data_buf` dentro de `chat_request(...)` como se muestra a continuación:
...
En esta función, se utiliza, `mbedtls_send_then_recv` para enviar la solicitud y recibir la respuesta.
...
> Principio: Cuando una solicitud es exitosa, se devuelve una URL. Al descargar y decodificar la imagen desde esta URL, esta puede mostrarse en pantalla.

Al igual que en el bloque `Código ChatGPT`, en la solicitud inicial se utiliza un *prompt* para obtener la URL de la imagen. Una vez que se adquiere la URL, se intenta descargar la imagen a un búfer local usando esa dirección.
...
<summary>Código ChatGPT y DALL·E</summary>

Para acceder al código completo y más reciente, consulta el repositorio de [SenseCAP Indicator OpenAI](https://github.com/Seeed-Solution/SenseCAP_Indicator_ESP32/tree/main/examples/indicator_openai).
...
4. **Guía del usuario del SenseCAP Indicator**: Esta guía proporciona información detallada sobre el hardware y software del SenseCAP Indicator. Puedes leerla [aquí](/Sensor/SenseCAP/SenseCAP_Indicator/Get_started_with_SenseCAP_Indicator).
5. **Guía de Chat Completions de OpenAI**: Si eres nuevo en la API de Chat, esta guía te ayudará a comenzar. Encuéntrala [aquí](https://platform.openai.com/docs/guides/chat/chat-completions-beta).
6. **Archivo `indicator_openai.c`**: Este archivo contiene las funciones principales para la integración con `ChatGPT` y `DALL·E`. Puedes verlo [aquí](https://raw.githubusercontent.com/Seeed-Solution/SenseCAP_Indicator_ESP32/main/examples/indicator_openai/main/model/indicator_openai.c).
7. **Guía de inicio para ESP-IDF**: Esta guía proporciona todos los pasos necesarios para configurar y usar ESP-IDF en tus proyectos. Accede a ella [aquí](https://docs.espressif.com/projects/esp-idf/en/latest/get-started/index.html).

## Soporte Técnico

**¿Necesitas ayuda con tu SenseCAP Indicator? ¡Estamos aquí para ayudarte!**

Si encuentras algún problema o tienes preguntas al seguir este tutorial, no dudes en contactar con nuestro equipo de soporte técnico. ¡Siempre estaremos encantados de ayudarte!

Visita nuestro [canal oficial de Discord de Seeed](https://discord.gg/kpY74apCWj) para hacer tus consultas o entra en las [discusiones de GitHub](https://github.com/Seeed-Solution/SenseCAP_Indicator_ESP32/discussions) para compartir lo que quieras.
